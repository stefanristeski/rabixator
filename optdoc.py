...
class Argument(ChildPattern):
    ...
    def parse(class_, source):
        ...
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['str'])):
            type = 'string'
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['int'])):
            type = 'int'
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['float'])):
            type = 'float'
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['file'])):
            type = 'File'
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['bool'])):
            type = 'boolean'
        if any(filter(lambda x: re.search(x, types[0], re.IGNORECASE), ['enu'])):
            type = 'enum'

        return {'name': name, 'value': value[0] if value else None, 'description': description, 'type': type}
...
class Option(ChildPattern):
    ...
    def parse(class_, option_description):
        ...
        for s in options.split():
            if s.isupper() or (s.startswith('<') and s.endswith('>')):
                type = s
            else:
                type = 'boolean'
            if s.startswith('--'):
